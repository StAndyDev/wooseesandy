...
export const WebSocketProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const hasSocketClosedRef = useRef(false); // un ref, pour savoir si le socket a été fermé avant de tester la connexion api (car state ne persiste pas entre les rerenders)
  ...
  const initWebSocket = async () => {
    try {
      ...
      socket.onopen = async () => {
        ...
        dispatch(addMessage('success', 'websocket', 'Connexion établie'));
        setSocketInstance(socket);
        if (hasSocketClosedRef.current) {
          console.log("reconnexion détectée, on check l'API");
          await checkApiConnection();
          hasSocketClosedRef.current = false; // reset le flag
        }
      }
      // --------- on message ------------
      ...
      socket.onclose = (event) => {
        if (!event.wasClean) {
          ...
          // reconnect
          setTimeout(() => {
            hasSocketClosedRef.current = true; // on marque que c’est une reconnexion
            initWebSocket();
